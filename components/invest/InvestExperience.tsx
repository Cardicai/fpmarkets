...
export default function InvestExperience({
  ...
  const handleInvest = () => {
    ...
    setAmount("");
    setInvestmentError(null);
    setInvestmentSuccess("Investment launched! ROI dashboard activated.");
  ...
  const roiAchieved = activeInvestment && activeInvestment.amount
    ? (earnings / activeInvestment.amount) * 100
    : 0;
  const roiProgress = roiTarget > 0 ? Math.min(earnings / roiTarget, 1) : 0;
    ...
    <main className="page page--invest">
      ...
      {activeInvestment && (
        <section className="section" id="dashboard">
          <div className="section__head">
            <span className="section__eyebrow">Live investment dashboard</span>
            <h2 className="section__title">
              Tracking {activeInvestment.project.title} at 69% ROI per day.
            </h2>
            <p className="section__subtitle">
              Earnings update every second so you can monitor the momentum of your
              chosen mission. Withdraw anytime your strategy hits the target.
            </p>
          </div>

          <div className="dashboard-grid">
            <div className="dashboard-panel dashboard-panel--insights">
              <div className="dashboard-panel__body dashboard-panel__body--live">
                <div>
                  <span>Strategy</span>
                ...
                <div>
                  ...
                  <p>Available balance {naira(availableBalance)}</p>
                </div>
                <div>
                  <span>Live earnings</span>
                  <strong>{naira(earnings)}</strong>
                  <p>{roiAchieved.toFixed(2)}% realised so far</p>
                </div>
                <div>
            ...
            <div className="dashboard-panel-wrapper">
              <div className="dashboard-panel dashboard-panel--actions dashboard-panel--live">
                <header>
                  <span>Execution timeline</span>
                  <Link href="#projects">Switch project</Link>
                </header>
                <div className="dashboard-actions dashboard-actions--live">
                  <div>
                    <h3>Cycle</h3>
                    <p>{activeInvestment.project.cycle}</p>
                    <span className="dashboard-tag">Running {elapsedLabel}</span>
                  </div>
                  <div>
                    <h3>Risk controls</h3>
                    <p>{activeInvestment.project.highlights.join(" • ")}</p>
                    <span className="dashboard-tag">Auto-withdraw on target</span>
                  </div>
                </div>
                <footer>
                  <div>
                  ...
                  <div>
                    <span>Payout ready at</span>
                    <strong>{naira(activeInvestment.amount + earnings)}</strong>
                  </div>
                </footer>
              </div>
            </div>
            ...
            <div className="dashboard-panel-wrapper">
              <div className="dashboard-panel dashboard-panel--rates dashboard-panel--live">
                <header>
                  <span>ROI tracker</span>
                  <strong>69% / day</strong>
                </header>
                <div className="dashboard-rate dashboard-rate--live">
                  <div>
                    <span>Per hour yield</span>
                    <strong>{naira((activeInvestment.amount * 0.69) / 24)}</strong>
                    <p>Auto-calculated from allocation.</p>
                  </div>
                  <div>
                    <span>Per minute yield</span>
                    <strong>{naira((activeInvestment.amount * 0.69) / (24 * 60))}</strong>
                    <p>Real-time ticker keeps climbing.</p>
                  </div>
                </div>
              </div>
            </div>
    ...
    <article className="project-card">
      ...
      <div className="project-card__body">
        ...
        <div className="project-card__actions">
          ...
          <Link href="#dashboard" className="project-card__link">
            See trade brief
          </Link>
        </div>
        {success && (
